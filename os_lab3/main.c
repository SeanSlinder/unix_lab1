 ...
 *
 * ВАЖНО (практика для защиты):
 * Реальные "distance to Earth" удобно подставлять как параметры при insmod,
 * взяв актуальные значения из источника (NASA/JPL/DSN и т.п.).
 * Тогда /proc/tsulab покажет текущую разницу на момент загрузки.
 *
 * Параметр dynamic=1 включает простую модель "обновления" расстояния:
 * distance(t) = distance_at_load + v_km_s * (t - load_time).
 * Это приближение (расстояние до Земли из-за орбитального движения Земли
 * не обязано расти монотонно), но для демонстрации подходит.
 */
